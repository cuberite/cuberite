return
{
	HOOK_DROPSPENSE =
	{
		...
		Desc = [[
			This callback is called whenever a {{cDropSpenser|dropspenser}} dropspenses an {{cItem|item}}. A plugin may decide to disallow 
      the move by returning true.
		]],
		Params =
		{
			{ Name = "World", Type = "{{cWorld}}", Notes = "World where the dropspenser resides" },
			{ Name = "DropSpenser", Type = "{{cDropSpenser}}", Notes = "The dropspenser that is pulling the item" },
