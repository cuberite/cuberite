...
constexpr BlockState AcaciaButton(const enum Face Face, const eBlockFace Facing, const bool Powered)
{
	if (Face == Face::Floor)
		...
		if (Facing == eBlockFace::BLOCK_FACE_ZM)
			...
			return 6457;
	else if (Facing == eBlockFace::BLOCK_FACE_ZM)
		if (Powered)
			return 6458;
		else
			return 6459;
...
constexpr BlockState AcaciaDoor(
...
constexpr BlockState ActivatorRail(const bool Powered, const enum Shape Shape)
{
	if (Powered)
		if (Shape == Shape::NorthSouth)
			...
			return 6828;
	else if (Shape == Shape::NorthSouth)
		return 6829;
	else if (Shape == Shape::EastWest)
		return 6830;
	else if (Shape == Shape::AscendingEast)
...
constexpr BlockState AndesiteWall(
	const enum East East,
		...
		if (North == North::None)
			if (South == South::None)
				if (Up)
					if (West == West::None)
						return 13138;
					else if (West == West::Low)
						return 13139;
					else
						return 13140;
				else if (West == West::None)
					return 13144;
				else if (West == West::Low)
					return 13145;
				else
...
constexpr BlockState BeeNest(const eBlockFace Facing, const unsigned char HoneyLevel)
{
	if (Facing == eBlockFace::BLOCK_FACE_ZM)
		if (HoneyLevel == 0)
			return 15776;
		else if (HoneyLevel == 1)
			return 15777;
		else if (HoneyLevel == 2)
			return 15778;
		else if (HoneyLevel == 3)
			return 15779;
...
constexpr BlockState BirchDoor(
	const eBlockFace Facing,
	const enum Half Half,
	const enum Hinge Hinge,
	const bool Open,
...
constexpr BlockState BlackstoneWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState BrickWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
{
constexpr BlockState BrownMushroomBlock(
	const bool Down,
	const bool East,
	const bool North,
	const bool South,
...
{
constexpr BlockState ChorusPlant(
	const bool Down,
	const bool East,
	const bool North,
	const bool South,
...
constexpr BlockState CobblestoneWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState CrimsonDoor(
	const eBlockFace Facing,
	const enum Half Half,
	const enum Hinge Hinge,
	const bool Open,
...
constexpr BlockState DarkOakDoor(
	const eBlockFace Facing,
	const enum Half Half,
	const enum Hinge Hinge,
	const bool Open,
...
constexpr BlockState DioriteWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState EndStoneBrickWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState GraniteWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
{
	if (Enabled)
		if (Facing == eBlockFace::BLOCK_FACE_YM)
			return 6728;
		else if (Facing == eBlockFace::BLOCK_FACE_ZM)
			return 6729;
		else if (Facing == eBlockFace::BLOCK_FACE_ZP)
			...
			return 6732;
	else if (Facing == eBlockFace::BLOCK_FACE_YM)
		return 6733;
	else if (Facing == eBlockFace::BLOCK_FACE_ZM)
		return 6734;
	else if (Facing == eBlockFace::BLOCK_FACE_ZP)
...
constexpr BlockState JungleDoor(
	const eBlockFace Facing,
	const enum Half Half,
	const enum Hinge Hinge,
	const bool Open,
...
constexpr BlockState MossyCobblestoneWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState MossyStoneBrickWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
{
constexpr BlockState MushroomStem(
	const bool Down,
	const bool East,
	const bool North,
	const bool South,
...
constexpr BlockState NetherBrickWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState PolishedBlackstoneBrickStairs(
	const eBlockFace Facing,
	const enum Half Half,
...
constexpr BlockState PolishedBlackstoneBrickWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState PolishedBlackstoneWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState PrismarineWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
{
constexpr BlockState RedMushroomBlock(
	const bool Down,
	const bool East,
	const bool North,
	const bool South,
...
constexpr BlockState RedNetherBrickWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState RedSandstoneWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState RedstoneWire(
	const enum East East,
	const enum North North,
...
constexpr BlockState SandstoneWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState SpruceDoor(
	const eBlockFace Facing,
	const enum Half Half,
	const enum Hinge Hinge,
	const bool Open,
...
constexpr BlockState StoneBrickWall(
	const enum East East,
	const enum North North,
	const enum South South,
	const bool Up,
...
constexpr BlockState WarpedDoor(
	const eBlockFace Facing,
	const enum Half Half,
	const enum Hinge Hinge,
	const bool Open,
