...
enum ENUM_BLOCK_TYPE : BLOCKTYPE
{
	...
	// Keep these two as the last values. Update the last block value to the last block with an id less than 255 when
	// adding another block IsValidBlock() depends on this (255 gets checked additionally because there is a gap. See
	// https://minecraft.wiki/w/Data_values#Block_IDs
...
enum ENUM_ITEM_TYPE : short
{
	...
	// Keep these two as the last values of the consecutive list, without a number - they will get their correct number
	// assigned automagically by C++ IsValidItem() depends on this!
	E_ITEM_NUMBER_OF_CONSECUTIVE_TYPES,  ///< Number of individual (different) consecutive itemtypes
	E_ITEM_MAX_CONSECUTIVE_TYPE_ID =
		E_ITEM_NUMBER_OF_CONSECUTIVE_TYPES - 1,  ///< Maximum consecutive ItemType number used
	...
	// Keep these two as the last values of the disc list, without a number - they will get their correct number
	// assigned automagically by C++ IsValidItem() depends on this!
...
/** Translates a blocktype string into blocktype. Takes either a number or an items.ini alias as input. Returns -1 on
 * failure. */
extern int BlockStringToType(const AString & a_BlockTypeString);

/** Translates an itemtype string into an item. Takes either a number, number^number, number:number or an items.ini
 * alias as input. Returns true if successful. */
extern bool StringToItem(const AString & a_ItemTypeString, cItem & a_Item);

/** Translates a full item into a string. If the ItemType is not recognized, the ItemType number is output into the
 * string. */
...
/** Translates a full item into a fully-specified string (including meta and count). If the ItemType is not recognized,
 * the ItemType number is output into the string. */
extern AString ItemToFullString(const cItem & a_Item);

/** Returns a cItem representing the item described in an IniFile's value; if the value doesn't exist, creates it with
