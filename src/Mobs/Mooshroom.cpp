...
cMooshroom::cMooshroom(void) :
	Super("Mooshroom", etMooshroom, "entity.cow.hurt", "entity.cow.death", "entity.cow.ambient", 0.9f, 1.4f)
{
}
...
void cMooshroom::OnRightClicked(cPlayer & a_Player)
{
	switch (a_Player.GetEquippedItem().m_ItemType)
	{
		...
		case Item::Shears:
		{
			...
			m_World->SpawnMob(GetPosX(), GetPosY(), GetPosZ(), etCow, false);
			Destroy();
		} break;
		default:
		{
			FLOGWARNING("{}: Item type not handled {}.", __FUNCTION__, NamespaceSerializer::From(a_Player.GetEquippedItem().m_ItemType));
		}
	}
}
