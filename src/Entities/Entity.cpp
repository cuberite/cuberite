...
bool cEntity::DoTakeDamage(TakeDamageInfo & a_TDI)
{
	...
	if ((a_TDI.Attacker != nullptr) && (a_TDI.Attacker->IsPlayer()))
	{
		...
		if (SharpnessLevel > 0)
		{
			a_TDI.FinalDamage += (ceil(1.25 * SharpnessLevel));
		}
		else if (SmiteLevel > 0)
		{
			if (IsMob())
			{
				...
				switch (Monster->GetMobType())
				{
					...
					case mtZombiePigman:
					{
						a_TDI.FinalDamage += (ceil(2.5 * SmiteLevel));
						break;
					}
					default: break;
				}
		...
		else if (BaneOfArthropodsLevel > 0)
		{
			if (IsMob())
			{
				...
				switch (Monster->GetMobType())
				{
					...
					case mtSilverfish:
					{
						a_TDI.FinalDamage += (ceil(2.5 * BaneOfArthropodsLevel));
	...
	m_Health -= static_cast<int>(a_TDI.FinalDamage);
