...
{
	m_World->DoWithEntityByID(a_AttackerID, [this, a_DamageType, a_RawDamage, a_KnockbackAmount](cEntity & a_Attacker)
		{
			cPawn * Attacker;
		...
		switch (GetWorld()->GetBlock(Position).Type())
		{
			...
			case BlockType::EndPortal:
			{
				...
				else
				{
					...
					if (IsPlayer())
					{
						spawnPos = Vector3d(100, 49, 0);  // Players spawn at different position than entities
						cPlayer * player = static_cast<cPlayer *>(this);
						// Player must face west, towards the main island
						player->SetYaw(90.0);
						player->SetPitch(0.0);
					}
