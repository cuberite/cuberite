	...
	switch (a_ClickAction)
	{
		case caRightClick:
		{
			...
			else if ((Slot.m_ItemType <= 0) || DraggingItem.IsEqual(Slot))
			{
				...
				auto & Handler = cItemHandler::For(Slot.m_ItemType);
		...
		case caLeftClick:
		{
			...
			{
				// Same type, add items:
				auto & Handler = cItemHandler::For(DraggingItem.m_ItemType);
...
void cSlotArea::DistributeStack(cItem & a_ItemStack, cPlayer & a_Player, bool a_ShouldApply, bool a_KeepEmptySlots, bool a_BackFill)
{
	for (int i = 0; i < m_NumSlots; i++)
	{
		...
		char NumFit = cItemHandler::For(Slot->m_ItemType).GetMaxStackSize() - Slot->m_ItemCount;
		if (NumFit <= 0)
		{
			// Full stack already
			continue;
...
void cSlotAreaCrafting::ClickedResult(cPlayer & a_Player)
{
	...
	{
		auto & Handler = cItemHandler::For(Result.m_ItemType);
...
void cSlotAreaAnvil::Clicked(cPlayer & a_Player, int a_SlotNum, eClickAction a_ClickAction, const cItem & a_ClickedItem)
{
	...
	if (!DraggingItem.IsEmpty())
	{
		if (!(DraggingItem.IsEqual(Slot) && ((DraggingItem.m_ItemCount + Slot.m_ItemCount) <= cItemHandler::For(Slot).GetMaxStackSize())))
		{
			return;
		}
	}
...
void cSlotAreaAnvil::UpdateResult(cPlayer & a_Player)
{
	...
	if (!Sacrifice.IsEmpty())
	{
		...
		if (Target.IsDamageable() && cItemHandler::For(Target).CanRepairWithRawMaterial(Sacrifice.m_ItemType))
		{
