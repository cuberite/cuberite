...
void cWorld::SpawnItemPickups(const cItems & a_Pickups, double a_BlockX, double a_BlockY, double a_BlockZ, double a_FlyAwaySpeed, bool IsPlayerCreated)
{
	...
	for (cItems::const_iterator itr = a_Pickups.begin(); itr != a_Pickups.end(); ++itr)
	{
		...
		auto Pickup = std::make_shared<cPickup>(
			a_BlockX, a_BlockY, a_BlockZ,
			*itr, IsPlayerCreated, SpeedX, SpeedY, SpeedZ
		);
		Pickup->Initialize(Pickup, *this);
	}
}
...
{
	auto FallingBlock = std::make_shared<cFallingBlock>(Vector3i(a_X, a_Y, a_Z), BlockType, BlockMeta);
	FallingBlock->Initialize(FallingBlock, *this);
...
UInt32 cWorld::SpawnExperienceOrb(double a_X, double a_Y, double a_Z, int a_Reward)
{
	...
	auto ExpOrb = std::make_shared<cExpOrb>(a_X, a_Y, a_Z, a_Reward);
	ExpOrb->Initialize(ExpOrb, *this);
...
UInt32 cWorld::SpawnMinecart(double a_X, double a_Y, double a_Z, int a_MinecartType, const cItem & a_Content, int a_BlockHeight)
{
	...
	{
		case E_ITEM_MINECART:             Minecart = std::make_shared<cRideableMinecart>(a_X, a_Y, a_Z, a_Content, a_BlockHeight); break;
		case E_ITEM_CHEST_MINECART:       Minecart = std::make_shared<cMinecartWithChest>(a_X, a_Y, a_Z); break;
		case E_ITEM_FURNACE_MINECART:     Minecart = std::make_shared<cMinecartWithFurnace>(a_X, a_Y, a_Z); break;
		case E_ITEM_MINECART_WITH_TNT:    Minecart = std::make_shared<cMinecartWithTNT>(a_X, a_Y, a_Z); break;
		case E_ITEM_MINECART_WITH_HOPPER: Minecart = std::make_shared<cMinecartWithHopper>(a_X, a_Y, a_Z); break;
		default:
		{
			return cEntity::INVALID_ID;
		}
	...
	Minecart->Initialize(Minecart, *this);
...
void cWorld::QueueSetChunkData(cSetChunkData & a_SetChunkData)
{
	...
	QueueTask(
		[a_SetChunkData = std::move(a_SetChunkData)](cWorld & a_World)
		{
			// TODO: make SetChunkData and all called functions const
			a_World.SetChunkData(const_cast<cSetChunkData &>(a_SetChunkData));
		}
	);
}
...
void cWorld::SetChunkData(cSetChunkData & a_SetChunkData)
{
	...
	for (const auto & Entity : a_SetChunkData.GetEntities())
	{
		Entity->Initialize(Entity, *this);
	}
...
void cWorld::AddEntity(const std::shared_ptr<cEntity> & a_Entity)
{
...
UInt32 cWorld::SpawnMobFinalize(const std::shared_ptr<cMonster> & a_Monster)
{
	...
	if (!a_Monster->Initialize(a_Monster, *this))
	{
		return cEntity::INVALID_ID;
	}
...
UInt32 cWorld::CreateProjectile(double a_PosX, double a_PosY, double a_PosZ, cProjectileEntity::eKind a_Kind, cEntity * a_Creator, const cItem * a_Item, const Vector3d * a_Speed)
{
	...
	if ((Projectile == nullptr) || !Projectile->Initialize(Projectile, *this))
	{
		return cEntity::INVALID_ID;
	}
