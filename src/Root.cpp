...
void cRoot::LoadWorlds(cDeadlockDetect & a_dd, cSettingsRepositoryInterface & a_Settings, bool a_IsNewIniFile)
{
	...
	m_WorldsByName.emplace(std::piecewise_construct, std::forward_as_tuple(m_DefaultWorldName), std::forward_as_tuple(m_DefaultWorldName, m_DefaultWorldName, a_dd, WorldNames));
	...
	*/
	...
	for (auto WorldNameValue : Worlds)
	{
		...
		m_WorldsByName.emplace(std::piecewise_construct, std::forward_as_tuple(WorldName), std::forward_as_tuple(WorldName.c_str(), WorldPath.c_str(), a_dd, WorldNames, Dimension, LinkTo));
	}  // for i - Worlds
