...
cProtocol180::cProtocol180(cClientHandle * a_Client, const AString & a_ServerAddress, uint16_t a_ServerPort, uint32_t a_State) :
	super(a_Client),
...
void cProtocol180::SendBlockChange(int a_BlockX, int a_BlockY, int a_BlockZ, BLOCKTYPE a_BlockType, NIBBLETYPE a_BlockMeta)
{
	...
	Pkt.WritePosition(a_BlockX, a_BlockY, a_BlockZ);
	Pkt.WriteVarInt(((uint32_t)a_BlockType << 4) | ((uint32_t)a_BlockMeta & 15));
}
...
void cProtocol180::SendBlockChanges(int a_ChunkX, int a_ChunkZ, const sSetBlockVector & a_Changes)
{
	...
	Pkt.WriteVarInt((uint32_t)a_Changes.size());
...
void cProtocol180::SendEntityEffect(const cEntity & a_Entity, int a_EffectID, int a_Amplifier, short a_Duration)
{
	...
	Pkt.WriteVarInt((uint32_t)a_Duration);
...
void cProtocol180::SendEntityEquipment(const cEntity & a_Entity, short a_SlotNum, const cItem & a_Item)
{
	...
	Pkt.WriteVarInt((uint32_t)a_Entity.GetUniqueID());
...
void cProtocol180::SendHealth(void)
{
	...
	Pkt.WriteVarInt((uint32_t)Player->GetFoodLevel());
...
void cProtocol180::SendMapColumn(int a_ID, int a_X, int a_Y, const uint8_t * a_Colors, unsigned int a_Length, unsigned int m_Scale)
{
	ASSERT(m_State == 3);  // In game mode?
...
void cProtocol180::SendPlayerListAddPlayer(const cPlayer & a_Player)
{
	...
	Pkt.WriteVarInt((uint32_t)a_Player.GetGameMode());
	Pkt.WriteVarInt((uint32_t)a_Player.GetClientHandle()->GetPing());
	Pkt.WriteBool(false);
}
...
void cProtocol180::SendExperience(void)
{
	...
	Pkt.WriteVarInt((uint32_t)Player->GetXpLevel());
	Pkt.WriteVarInt((uint32_t)Player->GetCurrentXp());
}
...
void cProtocol180::SendScoreboardObjective(const AString & a_Name, const AString & a_DisplayName, uint8_t a_Mode)
{
	ASSERT(m_State == 3);  // In game mode?
...
void cProtocol180::SendScoreUpdate(const AString & a_Objective, const AString & a_Player, cObjective::Score a_Score, uint8_t a_Mode)
{
	ASSERT(m_State == 3);  // In game mode?
	...
	if (a_Mode != 1)
	{
		Pkt.WriteVarInt((uint32_t) a_Score);
	}
}
...
void cProtocol180::SendSpawnObject(const cEntity & a_Entity, char a_ObjectType, int a_ObjectData, uint8_t a_Yaw, uint8_t a_Pitch)
{
	ASSERT(m_State == 3);  // In game mode?
...
void cProtocol180::SendTimeUpdate(int64_t a_WorldAge, int64_t a_TimeOfDay, bool a_DoDaylightCycle)
{
	ASSERT(m_State == 3);  // In game mode?
...
bool cProtocol180::CompressPacket(const AString & a_Packet, AString & a_CompressedData)
{
	...
	Buffer.WriteVarInt((uint32_t)a_Packet.size());
...
void cProtocol180::AddReceivedData(const char * a_Data, size_t a_Size)
{
	...
	for (;;)
	{
		...
		if (m_State == 3)
		{
			uint32_t NumBytesRead = m_ReceivedData.GetReadableSpace();
		...
		// The packet should have 1 uint8_t left in the buffer - the NUL we had added
...
bool cProtocol180::HandlePacket(cByteBuffer & a_ByteBuffer, uint32_t a_PacketType)
{
	switch (m_State)
	{
		case 1:
...
{
	uint32_t EncKeyLength, EncNonceLength;
...
void cProtocol180::HandlePacketBlockDig(cByteBuffer & a_ByteBuffer)
{
	...
	HANDLE_READ(a_ByteBuffer, ReadByte,  uint8_t, Face);
...
{
	HANDLE_READ(a_ByteBuffer, ReadVarInt, uint32_t,  PlayerID);
	HANDLE_READ(a_ByteBuffer, ReadChar,   char,    Action);
	HANDLE_READ(a_ByteBuffer, ReadVarInt, uint32_t,  JumpBoost);
...
{
	HANDLE_READ(a_ByteBuffer, ReadVarInt, uint32_t, KeepAliveID);
...
void cProtocol180::HandlePacketTabComplete(cByteBuffer & a_ByteBuffer)
{
	...
	if (HasPosition)
	{
		HANDLE_READ(a_ByteBuffer, ReadBEInt64, int64_t, Position);
	}
...
{
	HANDLE_READ(a_ByteBuffer, ReadVarInt, uint32_t, EntityID);
	HANDLE_READ(a_ByteBuffer, ReadVarInt, uint32_t, Type);
...
void cProtocol180::StartEncryption(const uint8_t * a_Key)
{
...
void cProtocol180::cPacketizer::WriteMobMetadata(const cMonster & a_Mob)
{
	switch (a_Mob.GetMobType())
	{
		...
		{
			WriteByte(0x30);
			WriteShort((uint8_t)(((const cEnderman &)a_Mob).GetCarriedBlock()));
			WriteByte(0x11);
