...
bool cListenThread::Initialize(const AString & a_DualPortsString, AString a_v4PortsString, AString a_v6PortsString)
{
	...
	{
		if ((!a_DualPortsString.empty()) && (!CreateSockets(cSocket::IPDual, a_DualPortsString)))
		{
			return false;
		}
	}
	...
	if ((!a_v4PortsString.empty()) && (!CreateSockets(cSocket::IPv4, a_v4PortsString)))
	{
		return false;
	}
	
	if ((!a_v6PortsString.empty()) && (!CreateSockets(cSocket::IPv6, a_v6PortsString)))
	{
		return false;
	}
	
	if ((a_DualPortsString.empty()) && (a_v4PortsString.empty()) && (a_v6PortsString.empty()))
	{
		return false;
	}
	return true;
