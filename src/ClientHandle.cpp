...
void cClientHandle::HandleEnchantItem(UInt8 a_WindowID, UInt8 a_Enchantment)
{
	...
	if (Item.EnchantByXPLevels(BaseEnchantmentLevel))
	{
		...
		if (IsCreative || ((m_Player->GetCurrentXp() >= XpRequired) && (LapisStack.m_ItemCount >= LapisRequired)))
		{
			// If they weren't creative then subtract xp and lapis
			if (!IsCreative)
			{
				/** We need to reduce the player's level by the amount of lapis required
				However we need to keep the resulting percentage filled the same */
				// Get current level and fill percent
				const auto CurrentLevel = m_Player->GetXpLevel();
				const auto TargetLevel = CurrentLevel - LapisRequired;
				const auto CurrentFillPercent = m_Player->GetXpPercentage();

				// Remove current xp and add amount for target level
				const auto DeltaForLevel = -m_Player->GetCurrentXp() + m_Player->XpForLevel(TargetLevel);
				// Add correct amount to get the same fill percent
