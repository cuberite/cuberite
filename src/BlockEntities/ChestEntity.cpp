...
void cChestEntity::OpenNewWindow(void)
{
	...
	if (!cChunkDef::IsRelCoordNeighborMoreThanChunkHeight(GetPosY()) && cBlockInfo::IsSolid(GetWorld()->GetBlock(GetPosX(), GetPosY() + 1, GetPosZ())))
	{
	...
	public:
		...
		{
			if (cChunkDef::IsRelCoordNeighborLessThanChunkHeight(a_Chest->GetPosY()) && cBlockInfo::IsSolid(a_Chest->GetWorld()->GetBlock(a_Chest->GetPosX(), a_Chest->GetPosY() + 1, a_Chest->GetPosZ())))
			{
				// Obstruction, don't open
				return false;
			}
