...
void cDropSpenserEntity::DropSpense(cChunk & a_Chunk)
{
	...
	if (SlotsCnt == 0)
	{
		...
		m_World->BroadcastSoundEffect(SoundEvent::BlockDispenserFail, m_Pos, 1.0f, 1.2f);
		return;
	}
	...
	m_World->BroadcastSoundEffect(SoundEvent::BlockDispenserDispense, m_Pos, 1.0f, 1.0f);
}
