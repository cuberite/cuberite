	...
	for (int x = 0; x <= 2; x++)
	{
		...
		for (int z = 0; z <= 2; z++)
		{
			...
			if ((x != 1) || (z != 1))
			{
				...
				// TODO: This may cause a lot of wasted calculations, instead of pulling data out of a single
				// (cChunkDesc) cache
			...
			if (NumTrees < 1)
			{
				...
				if (std::abs(
						m_Noise.IntNoise3D(BaseX * cChunkDef::Width + Pos.x, Pos.y, BaseZ * cChunkDef::Width + Pos.z)
					) <= NumTrees)
				{
		...
		if ((itr->m_ChunkX == a_ChunkX) && (itr->m_ChunkZ == a_ChunkZ) && (itr->m_RelY < cChunkDef::Height))
		{
			...
			switch (a_ChunkDesc.GetBlockType(itr->m_RelX, itr->m_RelY, itr->m_RelZ))
			{
				...
				CASE_TREE_OVERWRITTEN_BLOCKS:
				{
					...
					if ((cBlockInfo::IsSolid(itr->m_BlockType)) &&
...
double cStructGenTrees::GetNumTrees(int a_ChunkX, int a_ChunkZ, const cChunkDef::BiomeMap & a_Biomes)
{
...
void cStructGenLakes::CreateLakeImage(int a_ChunkX, int a_ChunkZ, int a_MaxLakeHeight, cBlockArea & a_Lake)
{
	...
	// Make a random position in the chunk by using a random 16 block XZ offset and random height up to chunk's max
	// height minus 6
	int MinHeight = std::max(a_MaxLakeHeight - 6, 2);
	int Rnd = m_Noise.IntNoise3DInt(a_ChunkX, 128, a_ChunkZ) / 11;
	// Random offset [-8 .. 8], with higher probability around 0; add up four three-bit-wide randoms [0 .. 28], divide
	// and subtract to get range
...
cStructGenDirectOverhangs::cStructGenDirectOverhangs(int a_Seed) :
	m_Noise1(a_Seed), m_Noise2(a_Seed + 1000)
{
}
	...
	for (int z = 0; z <= 16 / INTERPOL_Z; z++)
		for (int x = 0; x <= 16 / INTERPOL_X; x++)
		{
			FloorLo[INTERPOL_X * x + 17 * INTERPOL_Z * z] =
				...
				m_Noise2.IntNoise3DInt(BaseX + INTERPOL_X * x, BaseY, BaseZ + INTERPOL_Z * z) / 256;
		}  // for x, z - FloorLo[]
...
cStructGenDistortedMembraneOverhangs::cStructGenDistortedMembraneOverhangs(int a_Seed) :
	m_NoiseX(a_Seed + 1000), m_NoiseY(a_Seed + 2000), m_NoiseZ(a_Seed + 3000), m_NoiseH(a_Seed + 4000)
{
}
...
void cStructGenDistortedMembraneOverhangs::GenFinish(cChunkDesc & a_ChunkDesc)
{
	...
	for (int y = 50; y < 128; y++)
	{
		...
		{
			NOISE_DATATYPE NoiseZ =
				static_cast<NOISE_DATATYPE>(a_ChunkDesc.GetChunkZ() * cChunkDef::Width + z) / Frequency;
			for (int x = 0; x < cChunkDef::Width; x++)
			{
				NOISE_DATATYPE NoiseX =
					static_cast<NOISE_DATATYPE>(a_ChunkDesc.GetChunkX() * cChunkDef::Width + x) / Frequency;
				...
				int MembraneHeight = 96 -
					static_cast<int>((DistortY + m_NoiseH.CubicNoise2D(NoiseX + DistortX, NoiseZ + DistortZ)) * 30);
				if (MembraneHeight < y)
				{
