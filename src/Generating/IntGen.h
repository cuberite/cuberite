...
public:
	...
	typedef std::array<int, SizeX * SizeZ> Values;
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		for (int z = 0; z < SizeZ; z++)
		{
			...
			{
				a_Values[static_cast<size_t>(x + SizeX * z)] = (super::m_Noise.IntNoise2DInt(a_MinX + x, BaseZ) / 7) % Range;
			}
		}  // for z
	}
};
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		for (int z = 0; z < SizeZ; z++)
		{
			...
			for (int x = 0; x < SizeX; x++)
			{
				...
				a_Values[static_cast<size_t>(x + SizeX * z)] = ((rnd % 100) < m_Threshold) ? ((rnd / 101) % bgLandOceanMax + 1) : 0;
			}
		}
		...
		{
			a_Values[static_cast<size_t>(-a_MinX - a_MinZ * SizeX)] = bgTemperate;
		}
	}
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		std::array<int, m_LowerSizeX * m_LowerSizeZ> lowerData;
		...
		std::array<int, lowStepX * lowStepZ> cache;

		// Discreet-interpolate the values into twice the size:
		for (int z = 0; z < m_LowerSizeZ - 1; ++z)
		{
			size_t idx = static_cast<size_t>((z * 2) * lowStepX);
			int PrevZ0 = lowerData[static_cast<size_t>(z * m_LowerSizeX)];
			int PrevZ1 = lowerData[static_cast<size_t>((z + 1) * m_LowerSizeX)];

			for (int x = 0; x < m_LowerSizeX - 1; ++x)
			{
				int ValX1Z0 = lowerData[static_cast<size_t>(x + 1 + z * m_LowerSizeX)];
				int ValX1Z1 = lowerData[static_cast<size_t>(x + 1 + (z + 1) * m_LowerSizeX)];
		...
		for (int z = 0; z < SizeZ; ++z)
		{
			memcpy(a_Values.data() + z * SizeX, cache.data() + (z + (a_MinZ & 1)) * lowStepX + (a_MinX & 1), SizeX * sizeof(int));
		}
	}
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		for (int z = 0; z < SizeZ; z++)
		{
			...
			for (int x = 0; x < SizeX; x++)
			{
				int val   = lowerData[static_cast<size_t>(x + 1 + (z + 1) * m_LowerSizeX)];
				int above = lowerData[static_cast<size_t>(x + 1 +  z      * m_LowerSizeX)];
				int below = lowerData[static_cast<size_t>(x + 1 + (z + 2) * m_LowerSizeX)];
				int left  = lowerData[static_cast<size_t>(x     + (z + 1) * m_LowerSizeX)];
				int right = lowerData[static_cast<size_t>(x + 2 + (z + 1) * m_LowerSizeX)];
				...
				a_Values[static_cast<size_t>(x + z * SizeX)] = val;
			}
		}
	}
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		// Generate the underlying values:
		std::array<int, m_UnderlyingSizeX * m_UnderlyingSizeZ> lowerValues;
		...
		for (int z = 0; z < SizeZ; z++)
		{
			for (int x = 0; x < SizeX; x++)
			{
				int val   = lowerValues[static_cast<size_t>(x + 1 + (z + 1) * m_UnderlyingSizeX)];
				int above = lowerValues[static_cast<size_t>(x + 1 + z       * m_UnderlyingSizeX)];
				int below = lowerValues[static_cast<size_t>(x + 1 + (z + 2) * m_UnderlyingSizeX)];
				int left  = lowerValues[static_cast<size_t>(x     + (z + 1) * m_UnderlyingSizeX)];
				int right = lowerValues[static_cast<size_t>(x + 2 + (z + 1) * m_UnderlyingSizeX)];
				if (!IsBiomeOcean(val))
				{
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		for (int z = 0; z < SizeZ; z++)
		{
			for (int x = 0; x < SizeX; x++)
			{
				if (a_Values[static_cast<size_t>(x + z * SizeX)] == bgOcean)
				{
					int rnd = super::m_Noise.IntNoise2DInt(a_MinX + x, a_MinZ + z) / 7;
					if (rnd % 1000 < m_Chance)
					{
						a_Values[static_cast<size_t>(x + z * SizeX)] = (rnd / 1003) % bgLandOceanMax;
					}
				}
			}  // for x
		}  // for z
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		for (int z = 0; z < SizeZ; z++)
		{
			...
			for (int x = 0; x < SizeX; x++)
			{
				size_t val = static_cast<size_t>(a_Values[static_cast<size_t>(x + IdxZ)]);
				const cBiomesInGroups & Biomes = (val > bgfRare) ?
					rareBiomesInGroups[(val & (bgfRare - 1)) % rareBiomesInGroups.size()] :
					biomesInGroups[val % biomesInGroups.size()];
				int rnd = (super::m_Noise.IntNoise2DInt(x + a_MinX, z + a_MinZ) / 7);
				a_Values[static_cast<size_t>(x + IdxZ)] = Biomes.Biomes[rnd % Biomes.Count];
			}
		}
	}
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		for (int z = 0; z < SizeZ; z++)
		{
			...
			for (int x = 0; x < SizeX; x++)
			{
				size_t idx = static_cast<size_t>(x + idxZ);
...
public:
	...
	{
		// Generate the underlying data:
		std::array<int, UnderlyingSizeX * UnderlyingSizeZ> Cache;
		...
		for (int z = 0; z < SizeZ; z++)
		{
			for (int x = 0; x < SizeX; x++)
			{
				int Above = Cache[static_cast<size_t>(x + 1 + z * UnderlyingSizeX)];
				int Below = Cache[static_cast<size_t>(x + 1 + (z + 2) * UnderlyingSizeX)];
				int Left  = Cache[static_cast<size_t>(x +     (z + 1) * UnderlyingSizeX)];
				int Right = Cache[static_cast<size_t>(x + 2 + (z + 1) * UnderlyingSizeX)];
				int val   = Cache[static_cast<size_t>(x + 1 + (z + 1) * UnderlyingSizeX)];
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		for (int z = 0; z < SizeZ; z++)
		{
			for (int x = 0; x < SizeX; x++)
			{
				...
				{
					a_Values[static_cast<size_t>(x + z * SizeX)] = m_ToValue;
				}
				else
				{
					a_Values[static_cast<size_t>(x + z * SizeX)] = val;
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		for (int z = 0; z < SizeZ; z++)
		{
			for (int x = 0; x < SizeX; x++)
			{
				...
				if (rnd % 1000 < m_Chance)
				{
					size_t idx = static_cast<size_t>(x + SizeX * z);
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		for (size_t idx = 0; idx < static_cast<size_t>(SizeX * SizeZ); ++idx)
		{
...
public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, typename super::Values & a_Values) override
	{
		...
		for (int z = 0; z < SizeZ; z++)
		{
			for (int x = 0; x < SizeX; x++)
			{
				int biome = lowerValues[static_cast<size_t>(x + 1 + (z + 1) * m_LowerSizeX)];
				int above = lowerValues[static_cast<size_t>(x + 1 + z *       m_LowerSizeX)];
				int below = lowerValues[static_cast<size_t>(x + 1 + (z + 2) * m_LowerSizeX)];
				int left  = lowerValues[static_cast<size_t>(x +     (z + 1) * m_LowerSizeX)];
				int right = lowerValues[static_cast<size_t>(x + 2 + (z + 1) * m_LowerSizeX)];
				...
				a_Values[static_cast<size_t>(x + z * SizeX)] = biome;
			}  // for x
		}  // for z
	}
