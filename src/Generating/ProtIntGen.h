...
class cProtIntGenWithNoise : public cProtIntGen
{
...
class cProtIntGenChoice : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenChoice(int a_Seed, int a_Range) :
		Super(a_Seed), m_Range(a_Range)
	{
	}
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			...
			for (size_t x = 0; x < a_SizeX; x++)
			{
				a_Values[x + a_SizeX * z] =
					(Super::m_Noise.IntNoise2DInt(a_MinX + static_cast<int>(x), BaseZ) / 7) % m_Range;
			}
		}  // for z
	}
...
class cProtIntGenLandOcean : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
  ...
  public:
	...
	virtual void GetInts(int a_MinX, int a_MinZ, size_t a_SizeX, size_t a_SizeZ, int * a_Values) override
	{
		...
		if ((a_MinX <= 0) && (a_MinZ <= 0) && (a_MinX + static_cast<int>(a_SizeX) > 0) &&
			(a_MinZ + static_cast<int>(a_SizeZ) > 0))
		{
...
class cProtIntGenZoom : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenZoom(int a_Seed, Underlying a_UnderlyingGen) :
		Super(a_Seed), m_UnderlyingGen(std::move(a_UnderlyingGen))
	{
	}
...
class cProtIntGenSmooth : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenSmooth(int a_Seed, Underlying a_Underlying) :
		Super(a_Seed), m_Underlying(std::move(a_Underlying))
	{
	}
...
class cProtIntGenAvgValues : public cProtIntGen
{
	using Super = cProtIntGen;
	...
	virtual void GetInts(int a_MinX, int a_MinZ, size_t a_SizeX, size_t a_SizeZ, int * a_Values) override
	{
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			for (size_t x = 0; x < a_SizeX; x++)
			{
				...
				a_Values[x + a_SizeX * z] = (lowerData[idxLower] + lowerData[idxLower + 1] +
...
class cProtIntGenAvg4Values : public cProtIntGen
{
	using Super = cProtIntGen;
	...
	virtual void GetInts(int a_MinX, int a_MinZ, size_t a_SizeX, size_t a_SizeZ, int * a_Values) override
	{
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			for (size_t x = 0; x < a_SizeX; x++)
			{
				...
				a_Values[x + a_SizeX * z] =
					(1 * lowerData[idxLower1] + 2 * lowerData[idxLower1 + 1] + 2 * lowerData[idxLower1 + 2] +
					 1 * lowerData[idxLower1 + 3] + 2 * lowerData[idxLower2] + 32 * lowerData[idxLower2 + 1] +
					 32 * lowerData[idxLower2 + 2] + 2 * lowerData[idxLower2 + 3] + 2 * lowerData[idxLower3] +
					 32 * lowerData[idxLower3 + 1] + 32 * lowerData[idxLower3 + 2] + 2 * lowerData[idxLower3 + 3] +
					 1 * lowerData[idxLower4] + 2 * lowerData[idxLower4 + 1] + 2 * lowerData[idxLower4 + 2] +
					 1 * lowerData[idxLower4 + 3]) /
					148;
			}
		}
	}
...
template <int WeightCenter, int WeightCardinal, int WeightDiagonal> class cProtIntGenWeightAvg : public cProtIntGen
{
	using Super = cProtIntGen;
	...
	virtual void GetInts(int a_MinX, int a_MinZ, size_t a_SizeX, size_t a_SizeZ, int * a_Values) override
	{
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			for (size_t x = 0; x < a_SizeX; x++)
			{
				...
				size_t idxLower3 = idxLower1 + 2 * lowerSizeX;
				a_Values[x + a_SizeX * z] =
					(WeightDiagonal * lowerData[idxLower1] + WeightCardinal * lowerData[idxLower1 + 1] +
					 WeightDiagonal * lowerData[idxLower1 + 2] + WeightCardinal * lowerData[idxLower2] +
					 WeightCenter * lowerData[idxLower2 + 1] + WeightCardinal * lowerData[idxLower2 + 2] +
					 WeightDiagonal * lowerData[idxLower3] + WeightCardinal * lowerData[idxLower3 + 1] +
					 WeightDiagonal * lowerData[idxLower3 + 2]) /
...
class cProtIntGenRndChoice : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenRndChoice(int a_Seed, int a_ChancePct, int a_Min, int a_Range, Underlying a_Underlying) :
		Super(a_Seed), m_ChancePct(a_ChancePct), m_Min(a_Min), m_Range(a_Range), m_Underlying(std::move(a_Underlying))
	{
	}
	...
	virtual void GetInts(int a_MinX, int a_MinZ, size_t a_SizeX, size_t a_SizeZ, int * a_Values) override
	{
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			...
			for (size_t x = 0; x < a_SizeX; x++)
			{
				...
				{
					a_Values[x + a_SizeX * z] =
						m_Min + (Super::m_Noise.IntNoise2DInt(a_MinX + static_cast<int>(x), BaseZ) / 7) % m_Range;
				}
			}  // for x
		}  // for z
	}
...
class cProtIntGenAddRnd : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenAddRnd(int a_Seed, int a_HalfRange, Underlying a_Underlying) :
		Super(a_Seed), m_Range(a_HalfRange * 2 + 1), m_HalfRange(a_HalfRange), m_Underlying(std::move(a_Underlying))
	{
	}
	...
	virtual void GetInts(int a_MinX, int a_MinZ, size_t a_SizeX, size_t a_SizeZ, int * a_Values) override
	{
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			...
			{
				int noiseVal =
					((Super::m_Noise.IntNoise2DInt(a_MinX + static_cast<int>(x), NoiseZ) / 7) % m_Range) - m_HalfRange;
...
class cProtIntGenRndAvg : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenRndAvg(int a_Seed, int a_AvgChancePct, Underlying a_Underlying) :
		Super(a_Seed), m_AvgChancePct(a_AvgChancePct), m_Underlying(std::move(a_Underlying))
	{
	}
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			...
			for (size_t x = 0; x < a_SizeX; x++)
			{
				...
				if (((Super::m_Noise.IntNoise2DInt(a_MinX + static_cast<int>(x), NoiseZ) / 7) % 100) > m_AvgChancePct)
				{
					...
					a_Values[x + z * a_SizeX] = (lowerData[idxLower - 1] + lowerData[idxLower + 1] +
...
class cProtIntGenRndBetween : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			...
			for (size_t x = 0; x < a_SizeX; x++)
			{
				...
				if (((Super::m_Noise.IntNoise2DInt(a_MinX + static_cast<int>(x), NoiseZ) / 7) % 100) > m_AvgChancePct)
				{
					...
					int min = std::min(
						std::min(lowerData[idxLower - 1], lowerData[idxLower + 1]),
						std::min(lowerData[idxLower - lowerSizeX], lowerData[idxLower + lowerSizeX])
					);
					int max = std::max(
						std::max(lowerData[idxLower - 1], lowerData[idxLower + 1]),
						std::max(lowerData[idxLower - lowerSizeX], lowerData[idxLower + lowerSizeX])
					);
					a_Values[x + z * a_SizeX] = min +
						((Super::m_Noise.IntNoise2DInt(a_MinX + static_cast<int>(x), NoiseZ + 10) / 7) % (max - min + 1)
						);
				}
				else
				{
...
class cProtIntGenBeaches : public cProtIntGen
{
	using Super = cProtIntGen;
...
class cProtIntGenAddIslands : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
  ...
  public:
	...
	cProtIntGenAddIslands(int a_Seed, int a_Chance, Underlying a_Underlying) :
		Super(a_Seed), m_Chance(a_Chance), m_Underlying(std::move(a_Underlying))
	{
	}
	...
	virtual void GetInts(int a_MinX, int a_MinZ, size_t a_SizeX, size_t a_SizeZ, int * a_Values) override
	{
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			for (size_t x = 0; x < a_SizeX; x++)
			{
				...
				{
					int rnd =
						Super::m_Noise.IntNoise2DInt(a_MinX + static_cast<int>(x), a_MinZ + static_cast<int>(z)) / 7;
...
class cProtIntGenBiomeGroupEdges : public cProtIntGen
{
	using Super = cProtIntGen;
...
that biome. */
class cProtIntGenBiomes : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
	...
	virtual void GetInts(int a_MinX, int a_MinZ, size_t a_SizeX, size_t a_SizeZ, int * a_Values) override
	{
		...
		for (size_t z = 0; z < a_SizeZ; z++)
		{
			...
			for (size_t x = 0; x < a_SizeX; x++)
			{
				...
				int rnd =
					(Super::m_Noise.IntNoise2DInt(static_cast<int>(x) + a_MinX, static_cast<int>(z) + a_MinZ) / 7);
...
class cProtIntGenReplaceRandomly : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
  ...
  public:
	...
	cProtIntGenReplaceRandomly(int a_Seed, int a_From, int a_To, int a_Chance, Underlying a_Underlying) :
		Super(a_Seed), m_From(a_From), m_To(a_To), m_Chance(a_Chance), m_Underlying(std::move(a_Underlying))
	{
	}
...
class cProtIntGenMixRivers : public cProtIntGen
{
	using Super = cProtIntGen;

  public:
	cProtIntGenMixRivers(Underlying a_Biomes, Underlying a_Rivers) :
		m_Biomes(std::move(a_Biomes)), m_Rivers(std::move(a_Rivers))
	{
	}
...
class cProtIntGenRiver : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
...
class cProtIntGenAddToOcean : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenAddToOcean(int a_Seed, int a_Chance, int a_ToValue, Underlying a_Underlying) :
		Super(a_Seed), m_Chance(a_Chance), m_ToValue(a_ToValue), m_Underlying(std::move(a_Underlying))
	{
	}
...
class cProtIntGenSetRandomly : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
...
class cProtIntGenRareBiomeGroups : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
...
class cProtIntGenAlternateBiomes : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenAlternateBiomes(int a_Seed, Underlying a_Alterations, Underlying a_BaseBiomes) :
		Super(a_Seed), m_Alterations(std::move(a_Alterations)), m_BaseBiomes(std::move(a_BaseBiomes))
	{
	}
...
class cProtIntGenBiomeEdges : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;
...
class cProtIntGenMBiomes : public cProtIntGenWithNoise
{
	using Super = cProtIntGenWithNoise;

  public:
	cProtIntGenMBiomes(int a_Seed, Underlying a_Alteration, Underlying a_Underlying) :
		Super(a_Seed), m_Underlying(std::move(a_Underlying)), m_Alteration(std::move(a_Alteration))
	{
	}
