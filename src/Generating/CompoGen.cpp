...
void cCompoGenDebugBiomes::ComposeTerrain(cChunkDesc & a_ChunkDesc, const cChunkDesc::Shape & a_Shape)
{
	static BLOCKTYPE Blocks[] = {
		E_BLOCK_STONE,        E_BLOCK_COBBLESTONE, E_BLOCK_LOG,
		E_BLOCK_PLANKS,       E_BLOCK_SANDSTONE,   E_BLOCK_WOOL,
		E_BLOCK_COAL_ORE,     E_BLOCK_IRON_ORE,    E_BLOCK_GOLD_ORE,
		E_BLOCK_DIAMOND_ORE,  E_BLOCK_LAPIS_ORE,   E_BLOCK_REDSTONE_ORE,
		E_BLOCK_IRON_BLOCK,   E_BLOCK_GOLD_BLOCK,  E_BLOCK_DIAMOND_BLOCK,
		E_BLOCK_LAPIS_BLOCK,  E_BLOCK_BRICK,       E_BLOCK_MOSSY_COBBLESTONE,
		E_BLOCK_OBSIDIAN,     E_BLOCK_NETHERRACK,  E_BLOCK_SOULSAND,
		E_BLOCK_NETHER_BRICK, E_BLOCK_BEDROCK,
	};
...
void cCompoGenClassic::InitializeCompoGen(cIniFile & a_IniFile)
{
	...
	m_BlockMiddle =
		static_cast<BLOCKTYPE>(GetIniItemSet(a_IniFile, "Generator", "ClassicBlockMiddle", "dirt").m_ItemType);
	m_BlockBottom =
		static_cast<BLOCKTYPE>(GetIniItemSet(a_IniFile, "Generator", "ClassicBlockBottom", "stone").m_ItemType);
	m_BlockBeach =
		static_cast<BLOCKTYPE>(GetIniItemSet(a_IniFile, "Generator", "ClassicBlockBeach", "sand").m_ItemType);
	m_BlockBeachBottom =
		static_cast<BLOCKTYPE>(GetIniItemSet(a_IniFile, "Generator", "ClassicBlockBeachBottom", "sandstone").m_ItemType
		);
	m_BlockSea =
		static_cast<BLOCKTYPE>(GetIniItemSet(a_IniFile, "Generator", "ClassicBlockSea", "stationarywater").m_ItemType);
}
...
cCompoGenNether::cCompoGenNether(int a_Seed) :
	m_Noise1(a_Seed + 10), m_Noise2(a_Seed * a_Seed * 10 + a_Seed * 1000 + 6000), m_MaxThreshold(25000)
{
}
...
void cCompoGenNether::ComposeTerrain(cChunkDesc & a_ChunkDesc, const cChunkDesc::Shape & a_Shape)
{
	...
	// Interpolate the chunk in 16 * SEGMENT_HEIGHT * 16 "segments", each SEGMENT_HEIGHT blocks high and each linearly
	// interpolated separately. Have two buffers, one for the lowest floor and one for the highest floor, so that
	// Y-interpolation can be done between them Then swap the buffers and use the previously-top one as the
	// current-bottom, without recalculating it.
	...
	for (int z = 0; z <= 16 / INTERPOL_Z; z++)
		for (int x = 0; x <= 16 / INTERPOL_X; x++)
		{
			// We need to store the intermediate result in a volatile variable, otherwise gcc -O2 optimizes
			// through the undefined behavior in cNoise and produces different data than the other platforms / build
			// types (#4384)
			volatile int intermediate = m_Noise1.IntNoise3DInt(BaseX + INTERPOL_X * x, 0, BaseZ + INTERPOL_Z * z) *
		...
		for (int z = 0; z < 16; z++)
			for (int x = 0; x < 16; x++)
			{
				int Threshold = static_cast<int>(
					m_Noise1.CubicNoise2D(static_cast<float>(BaseX + x) / 75, static_cast<float>(BaseZ + z) / 75) *
					m_MaxThreshold
				);
	...
	for (int z = 0; z < 16; z++)
		for (int x = 0; x < 16; x++)
		{
			...
			NOISE_DATATYPE CeilingDisguise = (m_Noise1.CubicNoise2D(
				static_cast<float>(a_ChunkDesc.GetChunkX() * cChunkDef::Width + x) / 10,
				static_cast<float>(a_ChunkDesc.GetChunkZ() * cChunkDef::Width + z) / 10
			));
...
void cCompoGenCache::ComposeTerrain(cChunkDesc & a_ChunkDesc, const cChunkDesc::Shape & a_Shape)
{
	...
	{
		// LOGD("CompoGenCache: %d hits, %d misses, saved %.2f %%", m_NumHits, m_NumMisses, 100.0 * m_NumHits /
		// (m_NumHits + m_NumMisses)); LOGD("CompoGenCache: Avg cache chain length: %.2f",
		// static_cast<float>(m_TotalChain) / m_NumHits);
	}
#endif  // !NDEBUG
	...
	for (int i = 0; i < m_CacheSize; i++)
	{
		if ((m_CacheData[m_CacheOrder[i]].m_ChunkX != ChunkX) || (m_CacheData[m_CacheOrder[i]].m_ChunkZ != ChunkZ))
		{
			continue;
		}
		...
		memcpy(
			a_ChunkDesc.GetBlockMetasUncompressed(),
			m_CacheData[Idx].m_BlockMetas,
			sizeof(a_ChunkDesc.GetBlockMetasUncompressed())
		);
