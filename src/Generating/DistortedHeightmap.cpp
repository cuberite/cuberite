...
void cDistortedHeightmap::GenerateHeightArray(void)
{
	...
	NOISE_DATATYPE EndX =
		static_cast<NOISE_DATATYPE>((m_CurChunkCoords.m_ChunkX + 1) * cChunkDef::Width - 1) / m_FrequencyX;
	...
	NOISE_DATATYPE EndZ =
		static_cast<NOISE_DATATYPE>((m_CurChunkCoords.m_ChunkZ + 1) * cChunkDef::Width - 1) / m_FrequencyZ;
	...
	for (int z = 0; z < DIM_Z; z++)
	{
		...
		for (int y = 0; y < DIM_Y; y++)
		{
			...
			for (int x = 0; x < DIM_X; x++)
			{
				...
				DistHei[NoiseArrayIdx + x] =
					static_cast<NOISE_DATATYPE>(GetHeightmapAt(DistX, DistZ)) + static_cast<NOISE_DATATYPE>(0.5);
			}
		}
	}

	// Upscale the distorted heightmap into full dimensions:
	LinearUpscale3DArray(DistHei, DIM_X, DIM_Y, DIM_Z, m_DistortedHeightmap, INTERPOL_X, INTERPOL_Y, INTERPOL_Z);

	// DEBUG: Debug3DNoise(m_DistortedHeightmap, 17, 257, 17, fmt::format(FMT_STRING("DistortedHeightmap_{}_{}"),
	// m_CurChunkX, m_CurChunkZ));
}
	...
	for (int z = 0; z < DIM_Z; z++)
	{
		for (int x = 0; x < DIM_Z; x++)
		{
			GetDistortAmpsAt(
				...
				x * INTERPOL_X,
				z * INTERPOL_Z,
				m_DistortAmpX[x + DIM_X * z],
				m_DistortAmpZ[x + DIM_X * z]
			);
		}
	}
}
...
void cDistortedHeightmap::GetDistortAmpsAt(
