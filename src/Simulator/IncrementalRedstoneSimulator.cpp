...
void cIncrementalRedstoneSimulator::RedstoneAddBlock(int a_BlockX, int a_BlockY, int a_BlockZ, cChunk * a_Chunk, cChunk * a_OtherChunk)
{
	...
	PoweredBlocksList & PoweredBlocks = a_Chunk->GetRedstoneSimulatorData()->m_PoweredBlocks;
	...
	LinkedBlocksList & LinkedPoweredBlocks = a_Chunk->GetRedstoneSimulatorData()->m_LinkedBlocks;
	...
	SimulatedPlayerToggleableList & SimulatedPlayerToggleableBlocks = a_Chunk->GetRedstoneSimulatorData()->m_SimulatedPlayerToggleableBlocks;
	...
	RepeatersDelayList & RepeatersDelayList = a_Chunk->GetRedstoneSimulatorData()->m_RepeatersDelayList;
	...
	cCoordWithBlockAndBoolVector & RedstoneSimulatorChunkData = a_Chunk->GetRedstoneSimulatorData()->m_ChunkData;
	...
	cCoordWithBlockAndBoolVector & QueuedData = a_Chunk->GetRedstoneSimulatorData()->m_QueuedChunkData;
...
void cIncrementalRedstoneSimulator::SimulateChunk(float a_Dt, int a_ChunkX, int a_ChunkZ, cChunk * a_Chunk)
{
	...
	if (m_RedstoneSimulatorChunkData->m_ChunkData.empty() && m_RedstoneSimulatorChunkData->m_QueuedChunkData.empty())
	{
		return;
	}
...
void cIncrementalRedstoneSimulator::HandlePiston(int a_RelBlockX, int a_RelBlockY, int a_RelBlockZ)
{
	...
	{
		cBlockPistonHandler::ExtendPiston(BlockX, a_RelBlockY, BlockZ, &this->m_World);
	}
	else
	{
		cBlockPistonHandler::RetractPiston(BlockX, a_RelBlockY, BlockZ, &this->m_World);
	}
}
...
bool cIncrementalRedstoneSimulator::AreCoordsDirectlyPowered(int a_RelBlockX, int a_RelBlockY, int a_RelBlockZ, cChunk * a_Chunk)
{
	...
	for (PoweredBlocksList::const_iterator itr = a_Chunk->GetRedstoneSimulatorData()->m_PoweredBlocks.begin(); itr != a_Chunk->GetRedstoneSimulatorData()->m_PoweredBlocks.end(); ++itr)  // Check powered list
	{
...
void cIncrementalRedstoneSimulator::SetBlockPowered(int a_RelBlockX, int a_RelBlockY, int a_RelBlockZ, int a_RelSourceX, int a_RelSourceY, int a_RelSourceZ, unsigned char a_PowerLevel)
{
	...
	PoweredBlocksList & Powered = (Neighbour->GetRedstoneSimulatorData())->m_PoweredBlocks;  // We need to insert the value into the chunk who owns the block position
	...
	)
{
	...
	LinkedBlocksList & Linked = (Neighbour->GetRedstoneSimulatorData())->m_LinkedBlocks;
...
void cIncrementalRedstoneSimulator::SetSourceUnpowered(int a_SourceX, int a_SourceY, int a_SourceZ, cChunk * a_Chunk, bool a_IsFirstCall)
{
	...
	for (PoweredBlocksList::iterator itr = a_Chunk->GetRedstoneSimulatorData()->m_PoweredBlocks.begin(); itr != a_Chunk->GetRedstoneSimulatorData()->m_PoweredBlocks.end();)
	{
		if (itr->a_SourcePos.Equals(Vector3i(a_SourceX, a_SourceY, a_SourceZ)))
		{
			itr = a_Chunk->GetRedstoneSimulatorData()->m_PoweredBlocks.erase(itr);
			a_Chunk->SetIsRedstoneDirty(true);
			continue;
		}
		++itr;
	}
	for (LinkedBlocksList::iterator itr = a_Chunk->GetRedstoneSimulatorData()->m_LinkedBlocks.begin(); itr != a_Chunk->GetRedstoneSimulatorData()->m_LinkedBlocks.end();)
	{
		if (itr->a_SourcePos.Equals(Vector3i(a_SourceX, a_SourceY, a_SourceZ)))
		{
			itr = a_Chunk->GetRedstoneSimulatorData()->m_LinkedBlocks.erase(itr);
			a_Chunk->SetIsRedstoneDirty(true);
			continue;
		}
		++itr;
