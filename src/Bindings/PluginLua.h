...
class cPluginLua : public cPlugin
{
  ...
  public:
	...
	*/
	class cOperation
	{
	  public:
		cOperation(cPluginLua & a_Plugin) :
			m_Plugin(a_Plugin), m_Lock(a_Plugin.m_LuaState)
		{
		}
	...
	/** Returns true if the plugin contains the function for the specified hook type, using the old-style registration
	 * (#121) */
	...
	template <typename FnT, typename... Args> bool Call(FnT a_Fn, Args &&... a_Args)
	{
  ...
  */
	template <typename... Args> bool CallSimpleHooks(int a_HookType, Args &&... a_Args)
	{
		cOperation op(*this);
