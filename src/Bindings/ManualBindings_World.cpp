...
template <typename T> static bool GetStackVectorOr3Numbers(cLuaState & L, int a_Index, Vector3<T> & a_Return)
{
...
template <class BlockEntityType, BLOCKTYPE... BlockTypes> static int DoWithBlockEntityAt(lua_State * tolua_S)
{
	cLuaState L(tolua_S);
	int OffsetIndex;

	// Check params:
	if (!L.CheckParamSelf("cWorld") || !CheckParamVectorOr3Numbers(L, "Vector3<int>", 2, OffsetIndex) ||
		!L.CheckParamFunction(OffsetIndex) || !L.CheckParamEnd(OffsetIndex + 1))
	{
		return 0;
	}
	...
	// Get parameters:
	if (!L.GetStackValues(1, Self) || !GetStackVectorOr3Numbers(L, 2, Position) ||
	...
	if (!FnRef.IsValid())
	{
		return L.ApiParamError(
			fmt::format(FMT_STRING("Expected a valid callback function for parameter {}"), OffsetIndex)
		);
	}

	// Call the DoWith function:
	bool res = Self->DoWithBlockEntityAt(
		Position,
		[&L, &FnRef](cBlockEntity & a_BlockEntity)
		{
...
template <class Ty1, class Ty2, bool (Ty1::*ForEachFn)(const cBoundingBox &, cFunctionRef<bool(Ty2 &)>)>
static int ForEachInBox(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamUserType(2, "cBoundingBox") || !L.CheckParamFunction(3) ||
		!L.CheckParamEnd(4))
	{
		return 0;
	}
	...
	{
		return L.ApiParamError(
			fmt::format(FMT_STRING("Invalid world ({}) or boundingbox ({})"), fmt::ptr(Self), fmt::ptr(Box))
		);
	}
	if (!FnRef.IsValid())
	{
...
template <class BlockEntityType, BLOCKTYPE... BlockTypes> static int ForEachBlockEntityInChunk(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cWorld") || !L.CheckParamNumber(2, 3) || !L.CheckParamFunction(4) || !L.CheckParamEnd(5))
	{
		return 0;
	}
	...
	bool res = Self->ForEachBlockEntityInChunk(
		ChunkX,
		ChunkZ,
		[&L, &FnRef](cBlockEntity & a_BlockEntity)
		{
...
{
	/* Function signature:
	void BroadcastBlockAction(number a_BlockX, number a_BlockY, number a_BlockZ, number a_number1, number a_number2,
	number a_BlockType, cClientHandle a_Exclude)
	--or--
	void BroadcastBlockAction(Vector3<int> a_BlockPos, number a_Byte1, number a_Byte2, number a_BlockType, cClientHandle
	a_Exclude)
	*/

	cLuaState L(tolua_S);
	int Byte1Index;
	if (!L.CheckParamSelf("cWorld") || !CheckParamVectorOr3Numbers(L, "Vector3<int>", 2, Byte1Index) ||
	...
	if (!L.GetStackValues(1, Self) || !GetStackVectorOr3Numbers(L, 2, BlockPos) ||
...
{
	/* Function signature:
	void BroadcastSoundEffect(string a_SoundName, number a_X, number a_Y, number a_Z, number a_Volume, number a_Pitch,
	...
	*/
	...
	if (!L.GetStackValues(1, Self, SoundName) || !GetStackVectorOr3Numbers(L, 3, BlockPos) ||
...
static int tolua_cWorld_BroadcastSoundParticleEffect(lua_State * tolua_S)
{
	...
	--or--
	void BroadcastSoundParticleEffect(EffectID a_EffectID, number a_SrcX, number a_SrcY, number a_SrcZ, number a_Data,
	[cClientHandle a_Exclude])
	*/
	cLuaState L(tolua_S);
	int DataIndex;
	if (!L.CheckParamSelf("cWorld") || !L.CheckParamNumber(2) ||
		!CheckParamVectorOr3Numbers(L, "Vector3<int>", 3, DataIndex) || !L.CheckParamNumber(DataIndex))
	{
		return 0;
	}
	...
	{
		L.LogStackTrace();
		LOGWARN("BroadcastSoundParticleEffect with 3 position arguments is deprecated, use vector-parametered version "
				"instead.");
	}
	...
	if (!L.GetStackValues(1, World, EffectId) || !GetStackVectorOr3Numbers(L, 3, SrcPos) ||
...
{
	/* Function signature:
	World:BroadcastParticleEffect("Name", PosX, PosY, PosZ, OffX, OffY, OffZ, ParticleData, ParticleAmount,
	[ExcludeClient], [OptionalParam1], [OptionalParam2])
	--or--
	...
	*/
	...
	{
		L.LogStackTrace();
		LOGWARN("BroadcastParticleEffect with 3 position and 3 offset arguments is deprecated, use vector-parametered "
				"version instead.");
	}
	...
	if (!L.GetStackValues(1, World, Name) || !GetStackVectorOr3Numbers(L, 3, SrcPos) ||
...
static int tolua_cWorld_ChunkStay(lua_State * tolua_S)
{
	...
	*/
	...
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamTable(2) || !L.CheckParamFunctionOrNil(3, 4))
	{
		return 0;
	}
	...
	cLuaState::cOptionalCallbackPtr onChunkAvailable,
		onAllChunksAvailable;  // Callbacks may be unassigned at all - as a request to load / generate chunks
...
static int tolua_cWorld_DoExplosionAt(lua_State * tolua_S)
{
	...
	*/
	...
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamNumber(2, 5) || !L.CheckParamBool(6) ||
		!L.CheckParamNumber(7) || !L.CheckParamEnd(9))
	{
		return 0;
	}
...
static int tolua_cWorld_DoWithPlayerByUUID(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cWorld") || !L.CheckParamUUID(2) || !L.CheckParamFunction(3) || !L.CheckParamEnd(4))
	{
		return 0;
	}
...
static int tolua_cWorld_DoWithNearestPlayer(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cWorld") || !L.CheckParamUserType(2, "Vector3<double>") || !L.CheckParamNumber(3) ||
	...
	// Call the function:
	bool res = Self->DoWithNearestPlayer(
		Position,
		RangeLimit,
		[&](cPlayer & a_Player)
		{
...
static int tolua_cWorld_FastSetBlock(lua_State * tolua_S)
{
	...
	*/
	...
	int OffsetIndex;
	if (!L.CheckParamSelf("cWorld") || !CheckParamVectorOr3Numbers(L, "Vector3<int>", 2, OffsetIndex) ||
		!L.CheckParamNumber(OffsetIndex, OffsetIndex + 1) || !L.CheckParamEnd(OffsetIndex + 2))
	{
		return 0;
	}
	...
	// Read the params:
	if (!L.GetStackValues(1, World) || !GetStackVectorOr3Numbers(L, 2, Position) ||
...
static int tolua_cWorld_ForEachEntityInChunk(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamNumber(2, 3) || !L.CheckParamFunction(4) ||
		!L.CheckParamEnd(5))
	{
		return 0;
	}
	...
	// Call the DoWith function:
	bool res = Self->ForEachEntityInChunk(
		ChunkX,
		ChunkZ,
		[&](cEntity & a_Item)
		{
...
static int tolua_cWorld_ForEachLoadedChunk(lua_State * tolua_S)
{
	...
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamFunction(2))
	{
		return 0;
	}
...
static int tolua_cWorld_GetSignLines(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamNumber(2, 4) || !L.CheckParamEnd(5))
	{
		return 0;
	}
...
static int tolua_cWorld_GetTimeOfDay(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cWorld") || !L.CheckParamEnd(2))
	{
		return 0;
	}
...
static int tolua_cWorld_PrepareChunk(lua_State * tolua_S)
{
	...
	*/
	...
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamNumber(2, 3) || !L.CheckParamFunctionOrNil(4))
	{
		return 0;
	}

	// Wrap the Lua callback inside a C++ callback class:
	class cCallback : public cChunkCoordCallback
	{
	  public:
...
static int tolua_cWorld_QueueTask(lua_State * tolua_S)
{
	...
	World->QueueTask([Task](cWorld & a_World) { Task->Call(&a_World); });
	return 0;
}
...
static int tolua_cWorld_SetBlockMeta(lua_State * tolua_S)
{
	...
	*/
	...
	int OffsetIndex;
	if (!L.CheckParamSelf("cWorld") || !CheckParamVectorOr3Numbers(L, "Vector3<int>", 2, OffsetIndex) ||
		!L.CheckParamNumber(OffsetIndex) || !L.CheckParamEnd(OffsetIndex + 1))
	{
		return 0;
	}
	...
	// Read the params:
	if (!L.GetStackValue(1, World) || !GetStackVectorOr3Numbers(L, 2, Position) || !L.GetStackValue(OffsetIndex, Meta))
	{
		return 0;
	}
...
static int tolua_cWorld_SetSignLines(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamNumber(2, 4) || !L.CheckParamString(5, 8) ||
		!L.CheckParamEnd(9))
	{
		return 0;
	}
...
static int tolua_cWorld_SetTimeOfDay(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cWorld") || !L.CheckParamNumber(2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
...
static int tolua_cWorld_ScheduleTask(lua_State * tolua_S)
{
	...
	// Retrieve the args:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamNumber(2) || !L.CheckParamFunction(3))
	{
		return 0;
	}
	cWorld * World;
	...
	World->ScheduleTask(cTickTime(NumTicks), [Task](cWorld & a_World) { Task->Call(&a_World); });
	return 0;
}
...
static int tolua_cWorld_TryGetHeight(lua_State * tolua_S)
{
	...
	*/
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cWorld") || !L.CheckParamNumber(2, 3) || !L.CheckParamEnd(4))
	{
		return 0;
	}
