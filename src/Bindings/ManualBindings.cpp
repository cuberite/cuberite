...
class LuaCommandHandler : public cPluginManager::cCommandHandler
{
  public:
...
static int tolua_Clamp(lua_State * tolua_S)
{
	...
	if (NumArgs != 3)
	{
		return cManualBindings::lua_do_error(
			LuaState,
			"Error in function call '#funcname#': Requires 3 arguments, got %i",
			NumArgs
		);
	}
	...
	if (!lua_isnumber(LuaState, 1) || !lua_isnumber(LuaState, 2) || !lua_isnumber(LuaState, 3))
	{
		return cManualBindings::lua_do_error(
			LuaState,
			"Error in function call '#funcname#': Expected a number for parameters #1, #2 and #3"
		);
	}
...
{
	cLuaState S(tolua_S);
	if (!S.CheckParamString(1) || (!S.CheckParamNumber(2) && !S.CheckParamEnd(2)))
	{
...
{
	cLuaState S(tolua_S);
	if (!S.CheckParamString(1) || !S.CheckParamNumber(2))
	{
		cLuaState::LogStackTrace(tolua_S);
		return 0;
	}
...
{
	cLuaState S(tolua_S);
	if (!S.CheckParamString(1) || !S.CheckParamEnd(2))
	{
		cLuaState::LogStackTrace(tolua_S);
		return 0;
	}
...
static int tolua_StringSplitAndTrim(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamString(1, 2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
...
/** Prints the message to the console, optionally formatting it with a plugin name prefix if the second param on the Lua
 * stack is true. */
static void LogFromLuaStack(lua_State * tolua_S, const std::string_view a_Message, const eLogLevel a_LogLevel)
{
	...
	{
		Logger::LogSimple(
			fmt::format("[{}] {}", cManualBindings::GetLuaPlugin(tolua_S)->GetName(), a_Message),
			a_LogLevel
		);
	}
}
...
/** Retrieves a string log message from the first param on the Lua stack, optionally prefixes it with plugin name, and
...
static int tolua_LOG(lua_State * tolua_S)
{
	...
	if (tolua_isusertype(tolua_S, 1, "cCompositeChat", false, &err))
	{
		...
		if (CompositeChat != nullptr)  // isusertype returns true for nil values
		{
			LogFromLuaStack(
				tolua_S,
				CompositeChat->ExtractText(),
				cCompositeChat::MessageTypeToLogLevel(CompositeChat->GetMessageType())
			);
			return 0;
		}
	}
...
{
	cLuaState L(tolua_S);
	if (!L.CheckParamString(1) || !L.CheckParamEnd(2))
	{
		return 0;
	}
...
static int tolua_cFile_ChangeFileExt(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserTable(1, "cFile") || !L.CheckParamString(2, 3) || !L.CheckParamEnd(4))
	{
		return 0;
	}
...
static int tolua_cFile_CreateFolder(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserTable(1, "cFile") || !L.CheckParamString(2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
...
static int tolua_cFile_GetFolderContents(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState LuaState(tolua_S);
	if (!LuaState.CheckParamUserTable(1, "cFile") || !LuaState.CheckParamString(2) || !LuaState.CheckParamEnd(3))
	{
		return 0;
	}
...
{
	// API function no longer available:
	LOGWARNING("cPluginManager:GetPlugin() is no longer available. Use cPluginManager:DoWithPlugin() or "
			   "cPluginManager:CallPlugin() instead.");
	cLuaState::LogStackTrace(tolua_S);
	return 0;
}
...
static int tolua_cPluginManager_AddHook_DefFn(cPluginManager * a_PluginManager, cLuaState & S, int a_ParamIdx)
{
	...
	if (Plugin != cManualBindings::GetLuaPlugin(S))
	{
		...
		LOGWARNING(
			"cPluginManager.AddHook(): Invalid Plugin parameter, cannot add hook to foreign plugins. Hook not added."
		);
		S.LogStackTrace();
		return 0;
	}
...
static int tolua_cPluginManager_AddHook(lua_State * tolua_S)
{
	...
	cPluginManager:Get():AddHook(Plugin, HOOK_TYPE)            -- (4) old style (#121), accepted but complained about in
	the console cPluginManager.AddHook(Plugin, HOOK_TYPE)                  -- (5) old style (#121) mangled, accepted but
	complained about in the console
	*/
	...
	if (tolua_isusertype(S, 1, "cPluginManager", 0, &err))
	{
		...
		{
			LOGWARNING(
				"Malformed plugin, use cPluginManager.AddHook(HOOK_TYPE, CallbackFunction). Fixing the call for you."
			);
			S.LogStackTrace();
	...
	else if (tolua_isusertype(S, ParamIdx, "cPlugin", 0, &err) && lua_isnumber(S, ParamIdx + 1))
	{
		...
		LOGINFO("cPluginManager.AddHook(): Deprecated format used, use cPluginManager.AddHook(HOOK_TYPE, "
				"CallbackFunction) instead. Fixing the call for you.");
		S.LogStackTrace();
	...
	LOGWARNING(
		"cPluginManager:AddHook(): bad parameters. Expected HOOK_TYPE and CallbackFunction, got %s. Hook not added.",
		ParamDesc
	);
	S.LogStackTrace();
	return 0;
...
static int tolua_cPluginManager_ForEachCommand(lua_State * tolua_S)
{
	...
	*/
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cPluginManager") || !L.CheckParamFunction(2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
	...
	class cLuaCallback : public cPluginManager::cCommandEnumCallback
	{
	  public:
		cLuaCallback(cLuaState & a_LuaState, cLuaState::cRef & a_FnRef) :
			m_LuaState(a_LuaState), m_FnRef(a_FnRef)
		{
		}
...
static int tolua_cPluginManager_BindCommand(lua_State * a_LuaState)
{
	...
	*/
	...
	if (!tolua_iscppstring(L, idx, 0, &tolua_err) || !tolua_iscppstring(L, idx + 1, 0, &tolua_err) ||
		!tolua_iscppstring(L, idx + 3, 0, &tolua_err) || !tolua_isnoobj(L, idx + 4, &tolua_err))
	{
...
static int tolua_cPluginManager_BindConsoleCommand(lua_State * a_LuaState)
{
	...
	*/
	...
	if (!tolua_iscppstring(L, idx, 0, &tolua_err) ||  // Command
	...
	if (!lua_isfunction(L, idx + 1))
	{
		luaL_error(
			L,
			"\"BindConsoleCommand\" function expects a function as its 2nd parameter. Command-binding aborted."
		);
		return 0;
	}
	cPluginManager * self = cPluginManager::Get();
	...
	{
		LOGERROR(
			"\"BindConsoleCommand\": Cannot create a function reference. Console command \"%s\" not bound.",
			Command.c_str()
		);
		return 0;
	}
...
static int tolua_cPluginManager_CallPlugin(lua_State * tolua_S)
{
	...
	*/
	...
	cLuaState L(tolua_S);
	if (!L.CheckParamUserTable(1, "cPluginManager") || !L.CheckParamString(2, 3))
	{
		return 0;
	}
	...
	auto PluginCallback = [&](cPlugin & a_Plugin)
	{
		...
		NumReturns =
			static_cast<cPluginLua &>(a_Plugin).CallFunctionFromForeignState(FunctionName, L, 4, lua_gettop(L));
		return true;
	};
	...
	if (NumReturns < 0)
	{
		// The call has failed, there are zero return values. Do NOT return negative number (Lua considers that a
		// "yield")
		return 0;
	}
	return NumReturns;
...
static int tolua_cPluginManager_ExecuteConsoleCommand(lua_State * tolua_S)
{
	...
	*/
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserTable(1, "cPluginManager") || !L.CheckParamString(2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
...
static int tolua_cPlayer_GetPermissions(lua_State * tolua_S)
{
	...
	// Check the params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cPlayer") || !L.CheckParamEnd(2))
	{
		return 0;
	}
...
static int tolua_cPlayer_PlaceBlock(lua_State * tolua_S)
{
	// Check the params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cPlayer") || !L.CheckParamUserType(2, "Vector3<int>") || !L.CheckParamNumber(3, 4) ||
		!L.CheckParamEnd(5))
	{
		return 0;
	}
...
static int tolua_cPlayer_PermissionMatches(lua_State * tolua_S)
{
	...
	// Check the params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserTable(1, "cPlayer") || !L.CheckParamString(2, 3) || !L.CheckParamEnd(4))
	{
		return 0;
	}
...
template <class OBJTYPE, void (OBJTYPE::*SetCallback)(cLuaState::cCallbackPtr && a_CallbackFn)>
...
class cWebTabCallback : public cWebAdmin::cWebTabCallback
{
  public:
	...
	) override
	{
		...
		return m_Callback
			.Call(const_cast<HTTPRequest *>(&a_Request), a_UrlPath, cLuaState::Return, a_Content, a_ContentType);
	}
};
...
static int tolua_md5HexString(lua_State * tolua_S)
{
	...
	{
		Output << std::setw(2)
			   << static_cast<unsigned short>(md5Output[i]);  // Need to cast to a number, otherwise a char is output
	}
...
static int tolua_sha1HexString(lua_State * tolua_S)
{
	...
	{
		Output << std::setw(2)
			   << static_cast<unsigned short>(sha1Output[i]);  // Need to cast to a number, otherwise a char is output
	}
	lua_pushlstring(tolua_S, Output.str().c_str(), Output.str().size());
	return 1;
}
...
static int tolua_get_HTTPRequest_FormData(lua_State * a_LuaState)
{
	...
	for (auto & Data : FormData)
	{
		...
		// lua_pushlstring(a_LuaState, Data.second.Value.c_str(), Data.second.Value.size());  // Might contain binary
		// data
...
static int tolua_cUrlParser_GetDefaultPort(lua_State * a_LuaState)
{
	...
	// Check params:
	cLuaState L(a_LuaState);
	if (!L.CheckParamUserTable(1, "cUrlParser") || !L.CheckParamString(2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
...
{
	cLuaState S(L);
	if (!S.CheckParamSelf("cClientHandle") || !S.CheckParamString(2, 3) || !S.CheckParamEnd(4))
	{
		return 0;
	}
...
{
	cLuaState S(L);
	if (!S.CheckParamSelf("cClientHandle") || !S.CheckParamNumber(2, 3) || !S.CheckParamBool(4) || !S.CheckParamEnd(5))
	{
		return 0;
	}
...
{
	cLuaState S(L);
	if (!S.CheckParamSelf("cClientHandle") || !S.CheckParamEnd(2))
	{
		return 0;
	}
	cClientHandle * Client;
...
static int tolua_cClientHandle_GetUUID(lua_State * tolua_S)
{
	// Check the params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cClientHandle") || !L.CheckParamEnd(2))
	{
		return 0;
	}
...
static int tolua_cClientHandle_GenerateOfflineUUID(lua_State * tolua_S)
{
	// Check the params:
	cLuaState L(tolua_S);
	if (!L.CheckParamStaticSelf("cClientHandle") || !L.CheckParamString(2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
...
static int tolua_cClientHandle_IsUUIDOnline(lua_State * tolua_S)
{
	// Check the params:
	cLuaState L(tolua_S);
	if (!L.CheckParamStaticSelf("cClientHandle") || !L.CheckParamUUID(2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
...
static int tolua_cMobHeadEntity_SetOwner(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cMobHeadEntity") || !L.CheckParamUUID(2) || !L.CheckParamString(3, 5) || !L.CheckParamEnd(6))
	{
		return 0;
	}
...
static int tolua_cMobHeadEntity_GetOwnerUUID(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cMobHeadEntity") || !L.CheckParamEnd(2))
	{
		return 0;
	}
...
{
	cLuaState S(L);
	if (!S.CheckParamStaticSelf("cMojangAPI") || !S.CheckParamString(2) || !S.CheckParamUUID(3) || !S.CheckParamEnd(4))
	{
		return 0;
	}
...
{
	cLuaState S(L);
	if (!S.CheckParamStaticSelf("cMojangAPI") || !S.CheckParamUUID(2) || !S.CheckParamEnd(4))
	{
		return 0;
	}
...
{
	cLuaState S(L);
	if (!S.CheckParamStaticSelf("cMojangAPI") || !S.CheckParamString(2) || !S.CheckParamEnd(4))
	{
		return 0;
	}
...
{
	cLuaState S(L);
	if (!S.CheckParamStaticSelf("cMojangAPI") || !S.CheckParamTable(2) || !S.CheckParamEnd(4))
	{
		return 0;
	}
...
static int tolua_cMojangAPI_MakeUUIDDashed(lua_State * L)
{
	...
	// Check params:
	cLuaState S(L);
	if (!S.CheckParamStaticSelf("cMojangAPI") || !S.CheckParamUUID(2) || !S.CheckParamEnd(3))
	{
		return 0;
	}
...
static int tolua_cMojangAPI_MakeUUIDShort(lua_State * L)
{
	...
	// Check params:
	cLuaState S(L);
	if (!S.CheckParamUserTable(1, "cMojangAPI") || !S.CheckParamUUID(2) || !S.CheckParamEnd(3))
	{
		return 0;
	}
...
static int tolua_cItem_EnchantByXPLevels(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cItem") || !L.CheckParamNumber(2))
	{
		return 0;
	}
...
static int tolua_set_cItem_m_LoreTable(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cItem") || !L.CheckParamTable(2))
	{
		return 0;
	}
...
{
	tolua_Error tolua_err;
	if (!tolua_isusertype(L, 1, "const cItemGrid", 0, &tolua_err) || !tolua_isnumber(L, 2, 0, &tolua_err) ||
...
class cLuaBlockTracerCallbacks : public cBlockTracer::cCallbacks
{
  public:
		...
		override
	{
		bool res = false;
		if (m_Callbacks
				->CallTableFn("OnNextBlock", a_BlockPos, a_BlockType, a_BlockMeta, a_EntryFace, cLuaState::Return, res))
		{
			return res;
		}
	...
	virtual void OnNoMoreHits(void) override { m_Callbacks->CallTableFn("OnNoMoreHits"); }

	virtual void OnNoChunk(void) override { m_Callbacks->CallTableFn("OnNoChunk"); }
...
static int tolua_cLineBlockTracer_FirstSolidHitTrace(lua_State * tolua_S)
{
	...
	cLineBlockTracer:FirstSolidHitTrace(World, StartX, StartY, StartZ, EndX, EndY, EndZ) -> bool, [Vector3d, Vector3i,
	eBlockFace]  // Canonical cLineBlockTracer:FirstSolidHitTrace(World, Start, End) -> bool, [Vector3d, Vector3i,
	eBlockFace]                                // Canonical cLineBlockTracer.FirstSolidHitTrace(World, StartX, StartY,
	StartZ, EndX, EndY, EndZ) -> bool, [Vector3d, Vector3i, eBlockFace] cLineBlockTracer.FirstSolidHitTrace(World,
	Start, End) -> bool, [Vector3d, Vector3i, eBlockFace]
	*/
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(idx, "cWorld"))
	{
		return 0;
	}
	...
	if (L.IsParamNumber(idx + 1))
	{
		...
		if (!L.CheckParamNumber(idx + 1, idx + 6) || !L.CheckParamEnd(idx + 7))
		{
			return 0;
		}
		// Get the params:
	...
	if (L.IsParamVector3(idx + 1))
	{
		...
		if (!L.CheckParamVector3(idx + 1, idx + 2) || !L.CheckParamEnd(idx + 3))
		{
			return 0;
		}
		// Get the params:
	...
	tolua_error(
		L,
		"cLineBlockTracer:FirstSolidHitTrace(): Invalid parameters, expected either a set of coords, or two Vector3's",
		nullptr
	);
	return 0;
}
...
static int tolua_cLineBlockTracer_LineOfSightTrace(lua_State * tolua_S)
{
	...
	*/
	...
	{
		// This is the number variant of the call:
		if (!L.CheckParamNumber(idx + 1, idx + 6) ||
		...
		L.Push(cLineBlockTracer::LineOfSightTrace(
			*world,
			Vector3d(startX, startY, startZ),
			Vector3d(endX, endY, endZ),
			lineOfSight
	...
	{
		// This is the Vector3-based variant of the call:
		if (!L.CheckParamVector3(idx + 1, idx + 2) ||
			// Optional param lineOfSight is not checked
	...
	tolua_error(
		L,
		"cLineBlockTracer:LineOfSightTrace(): Invalid parameters, expected either a set of coords, or two Vector3's",
		nullptr
	);
	return 0;
}
...
static int tolua_cLineBlockTracer_Trace(lua_State * tolua_S)
{
	...
	*/
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(idx, "cWorld") || !L.CheckParamTable(idx + 1))
	{
		return 0;
	}
	...
	if (L.IsParamNumber(idx + 2) && L.IsParamNumber(idx + 3) && L.IsParamNumber(idx + 4) && L.IsParamNumber(idx + 5) &&
		L.IsParamNumber(idx + 6) && L.IsParamNumber(idx + 7) && L.CheckParamEnd(idx + 8))
	{
		if (!L.GetStackValues(idx, world, callbacks, start.x, start.y, start.z, end.x, end.y, end.z))
		{
			LOGWARNING(
				"cLineBlockTracer:Trace(): Cannot read parameters (starting at idx %d), aborting the trace.",
				idx
			);
			L.LogStackTrace();
			L.LogStackValues("Values on the stack");
	...
	else if (L.IsParamVector3(idx + 2) && L.IsParamVector3(idx + 3) && L.CheckParamEnd(idx + 4))
	{
...
static int tolua_cLuaWindow_new(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserTable(1, "cLuaWindow") || !L.CheckParamNumber(2, 4) || !L.CheckParamString(5) ||
		!L.CheckParamEnd(6))
	{
		return 0;
	}
...
static int tolua_cRoot_DoWithPlayerByUUID(lua_State * tolua_S)
{
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamSelf("cRoot") || !L.CheckParamUUID(2) || !L.CheckParamFunction(3) || !L.CheckParamEnd(4))
	{
		return 0;
	}
	...
	// Call the function:
	bool res = Self->DoWithPlayerByUUID(
		PlayerUUID,
		[&](cPlayer & a_Player)
		{
			bool ret = false;
...
{
	cLuaState L(tolua_S);
	if (!L.CheckParamUserTable(1, "cRoot") || !L.CheckParamUserType(2, "const cItem") ||
		!L.CheckParamUserType(3, "const cItem") || !L.CheckParamEnd(4))
	{
		return 0;
	}
...
{
	cLuaState L(tolua_S);
	if (!L.CheckParamUserTable(1, "cRoot") || !L.CheckParamUserType(2, "const cItem") || !L.CheckParamEnd(3))
	{
		return 0;
	}
...
{
	cLuaState L(a_LuaState);
	if (!L.CheckParamSelf("cServer") || !L.CheckParamString(2, 3) || !L.CheckParamNumber(4) || !L.CheckParamEnd(5))
	{
		return 0;
	}
...
static int tolua_cServer_ScheduleTask(lua_State * a_LuaState)
{
	...
	// Retrieve the args:
	cLuaState L(a_LuaState);
	if (!L.CheckParamUserType(1, "cServer") || !L.CheckParamNumber(2) || !L.CheckParamFunction(3))
	{
		return 0;
	}
	cServer * Server;
	...
	Server->ScheduleTask(cTickTime(NumTicks), [Task](cServer & a_Server) { Task->Call(&a_Server); });
	return 0;
}
...
{
	cLuaState S(L);
	if (!S.CheckParamUserType(1, "cScoreboard") || !S.CheckParamEnd(2))
	{
		return 0;
	}
...
static int tolua_cHopperEntity_GetOutputBlockPos(lua_State * tolua_S)
{
	...
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cHopperEntity") || !L.CheckParamNumber(2) || !L.CheckParamEnd(3))
	{
		return 0;
	}
	...
	*/
	...
	else
	{
		...
		if (!L.GetStackValues(1, bbox, pt1, pt2))  // Try the regular signature
		{
			...
			tolua_error(
				a_LuaState,
				"Invalid function params. Expected either bbox:CalcLineIntersection(pt1, pt2) or "
				"cBoundingBox:CalcLineIntersection(min, max, pt1, pt2).",
				nullptr
			);
			return 0;
		}
...
static int tolua_cChunkDesc_GetBlockTypeMeta(lua_State * a_LuaState)
{
	...
	*/
	...
	if (!L.GetStackValues(1, self, relX, relY, relZ))
	{
		...
		tolua_error(
			a_LuaState,
			"Invalid function params. Expected chunkDesc:GetBlockTypeMeta(relX, relY, relZ)",
			nullptr
		);
		return 0;
	}
	...
	*/
	...
	if (!lua_isnone(a_LuaState, 3))
	{
		if (!L.GetStackValue(3, messageTypeInt))
		{
			...
			tolua_error(
				a_LuaState,
				"Invalid type of the MessageType parameter (2) in cCompositeChat constructor.",
				&err
			);
			return 0;
		}
...
static int tolua_cCompositeChat_AddShowAchievementPart(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cCompositeChat") || !L.CheckParamString(2, 3))
	{
		return 0;
	}
	...
	if (self == nullptr)
	{
		tolua_error(
			tolua_S,
			"invalid 'self' in function call cCompositeChat::AddShowAchievementPart(string, string, string) const.",
			nullptr
		);
		return 0;
	}
...
static int tolua_cCompositeChat_Clear(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cCompositeChat") || !L.CheckParamEnd(2))
	{
		return 0;
	}
	cCompositeChat * self = static_cast<cCompositeChat *>(tolua_tousertype(tolua_S, 1, nullptr));
...
static int tolua_cCompositeChat_SetMessageType(lua_State * tolua_S)
{
	...
	// Check params:
	cLuaState L(tolua_S);
	if (!L.CheckParamUserType(1, "cCompositeChat") || !L.CheckParamNumber(2))
	{
		return 0;
	}
	cCompositeChat * self = static_cast<cCompositeChat *>(tolua_tousertype(tolua_S, 1, nullptr));
...
static int tolua_cCuboid_Assign(lua_State * tolua_S)
{
	...
	{
		LOGWARNING(
			"cCuboid:Assign(x1, y1, z1, x2, y2, z2) is deprecated, use cCuboid:Assign(Vector3i, Vector3i) instead."
		);
		L.LogStackTrace();
