...
void cChunk::ApplyWeatherToTop()
{
	...
	switch (GetBiomeAt(X, Z))
	{
		...
		case biTaigaHills:
		{
			...
			cBlockTypeAndMetaReader BlockTypeAndMetaReader[4];
			if (m_World->IsDeepSnowEnabled() && (TopBlock == E_BLOCK_SNOW))
			{
				int MaxSize = 7;
				UnboundedRelGetBlockData(X - 1, Height, Z,     BlockTypeAndMetaReader[0]);
				UnboundedRelGetBlockData(X + 1, Height, Z,     BlockTypeAndMetaReader[1]);
				UnboundedRelGetBlockData(X,     Height, Z - 1, BlockTypeAndMetaReader[2]);
				UnboundedRelGetBlockData(X,     Height, Z + 1, BlockTypeAndMetaReader[3]);
				for (int i = 0; i < 4; i++)
				{
					switch (BlockTypeAndMetaReader[i].getType())
					{
						case E_BLOCK_AIR:
						{
							MaxSize = 0;
						...
						case E_BLOCK_SNOW:
						{
							MaxSize = std::min(BlockTypeAndMetaReader[i].getMeta() + 1, MaxSize);
							break;
						}
					}
				}
...
void cChunk::GrowMelonPumpkin(int a_RelX, int a_RelY, int a_RelZ, BLOCKTYPE a_BlockType, MTRand & a_TickRandom)
{
	...
	bool IsValid;
	cBlockTypeReader BlockTypeReader[4];
	IsValid =            UnboundedRelGetBlockData(a_RelX + 1, a_RelY, a_RelZ,     BlockTypeReader[0]);
	IsValid = IsValid && UnboundedRelGetBlockData(a_RelX - 1, a_RelY, a_RelZ,     BlockTypeReader[1]);
	IsValid = IsValid && UnboundedRelGetBlockData(a_RelX,     a_RelY, a_RelZ + 1, BlockTypeReader[2]);
	IsValid = IsValid && UnboundedRelGetBlockData(a_RelX,     a_RelY, a_RelZ - 1, BlockTypeReader[3]);
	if (
		!IsValid || 
		(BlockTypeReader[0].getValue() == ProduceType) || 
		(BlockTypeReader[1].getValue() == ProduceType) || 
		(BlockTypeReader[2].getValue() == ProduceType) || 
		(BlockTypeReader[3].getValue() == ProduceType)
	)
	{
	...
	switch (BlockTypeReader[CheckType].getValue())
	{
		case E_BLOCK_AIR:
		case E_BLOCK_SNOW:
	...
	switch (Soil.getValue())
	{
		...
		case E_BLOCK_FARMLAND:
		{
			...
			LOG("Growing melon/pumpkin overwriting %s, growing on %s",
				ItemTypeToString(BlockTypeReader[CheckType].getValue()).c_str(),
				ItemTypeToString(Soil.getValue()).c_str()
			);
...
void cChunk::GrowSugarcane(int a_RelX, int a_RelY, int a_RelZ, int a_NumBlocks)
{
	...
	{
		if (UnboundedRelGetBlockData(a_RelX, Top + i, a_RelZ, BlockTypeReader) && (BlockTypeReader.getValue() == E_BLOCK_AIR))
		{
